...
- **Distraction-free reading mode** *(shipped July 2026; went article-only at every width later that month)* — focus clears everything but the open article, on desktop and phone alike: navigation, sibling rows, and list controls hide, the sticky header shrinks to a slim strip (reading progress, status feedback, exit), and the article drops its list indent for the full column width. Originally focus only removed navigation chrome, which on a phone is a slim top bar — it barely changed the picture, and once the phone treatment existed the split behavior wasn't worth keeping. The open article keeps its on-screen position across the toggle; status feedback that normally sits beside Mark all read surfaces in the strip instead (`role="status"` so it's announced); `j`/`k`/`space` keep working and anchor the next article's title just below the strip. Focus is committed reading, not triage, so the action bar bracketing the top of the article steps back too (the bottom copy stays), and at `md`+ the strip carries the open article's title for context after the headline scrolls away. Toggle with `f` or from the strip; Escape also exits, and with nothing open `f` starts focused reading on the article `j` would open. The request is sticky within a view: closing an article shows the full list again, opening the next returns to focus — it clears on exit or when navigating to another view. In unread-only views `k` walks back through everything read this visit; at the top it says so in the strip rather than failing silently (earlier reads live under Show read).
