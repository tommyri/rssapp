...
- **Distraction-free reading mode** *(shipped July 2026; phone behavior deepened later that month)* — collapse the sidebar/chrome for a full-focus single column. Exit from the reader header or with Escape. On phones (below `md`), where hiding navigation alone changed little, focus clears everything but the open article: sibling rows and list controls hide, the sticky header shrinks to a slim strip (reading progress, status feedback, exit), and the article drops its list indent for the full column width. The open article keeps its on-screen position across the toggle, and status feedback that normally sits beside Mark all read surfaces in the strip instead.
