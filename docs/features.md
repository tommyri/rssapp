...
- **In-article rendering polish** *(mostly shipped July 2026)* — shipped: **code-block syntax highlighting** (highlight.js common build, auto-detected — the sanitizer strips class attributes so language hints don't survive ingest; dynamically imported only when an expanded article contains a `<pre>`, so prose never pays for it; a restrained three-hue theme in globals.css rather than a stock theme) and a **click-to-zoom lightbox** for images (portal-rendered — the row entrance animation's lingering transform would otherwise trap `position: fixed` inside the row). **Lazy-load images turned out to be already shipped**: the sanitizer stamps `loading="lazy"` on every `<img>` at ingest (verified 305/305 stored items). All rendering lives in `ArticleContent`, shared by feed items, extracted full content, and saved pages. Still open: **click-to-load embeds** (YouTube/tweets as light placeholders) — touches the sanitizer's iframe policy, deferred to its own slice.
