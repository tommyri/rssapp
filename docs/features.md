...
- **Podcast / audio enclosures** *(shipped July 2026)* — recognizes audio attachments in RSS and JSON Feed entries, retains them through backup and restore, and presents a native inline player when an expanded article has an episode to listen to. Feed media remains online-only; offline reading intentionally stores readable text rather than arbitrary audio files.
