...
- **PWA + offline reading** *(foundation + Read later download shipped July 2026)* — installable app shell and a device-local offline library. Choose **Keep offline** on an article or saved page, or download the newest 50 readable **Read later** entries from `/offline`, then read their sanitized text without a connection. Deliberately not cached: dynamic authenticated reader pages, arbitrary images, and third-party embeds. Still open: scheduled download sets, pull-to-refresh, and queued offline mutations/sync.
